...
?>
<div class="trailler_input_area">
	<div class="container trailler_input">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="first">Trailler Fee Calculation</h2>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<form id="trailler_form" action="<?php htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon">Trailler Weight</div>
							<input class="form-control"  min="1000" max="15000" type="number" name="weight" <?php if($trailler_weight == ""){}else{ echo "value = ".$trailler_weight;}?> id="" placeholder="Enter your Trailler Weight"/>
...
function reg_non_hire_cal($trailler_weight){
	if($trailler_weight <= 3500){$reg_fee = 28000;}
	else if($trailler_weight <= 7500){$reg_fee = 56000;}
	else if($trailler_weight <= 12500){$reg_fee = 70000;}
	else if($trailler_weight > 12500){$reg_fee = 77000;}
	else{$reg_fee = "";}
...
function reg_hire_cal($trailler_weight){
	if($trailler_weight <= 3500){$reg_hire_fee = 4600;}
	else if($trailler_weight <= 7500){$reg_hire_fee = 13800;}
	else if($trailler_weight <= 12500){$reg_hire_fee = 17250;}
	else if($trailler_weight > 12500){$reg_hire_fee = 23000;}
	else{$reg_hire_fee = "";}
...
function tax_token_hire_cal($trailler_weight){
	if($trailler_weight <= 3500){$tax_token_hire_fee = 2000;}
	else if($trailler_weight <= 7500){
								$xtra_weight = $trailler_weight - 3500;
	...
	else if($trailler_weight <= 12500){
								$xtra_weight = $trailler_weight - 7500;
								$xtra_per_500_kg = $xtra_weight / 500;
								...
								$xtra_weight = $trailler_weight - 12500;
								$xtra_per_500_kg = $xtra_weight / 500;
								$extra_fee_per_500_kg = $xtra_per_500_kg * 500;
...
function tax_token_non_hire_cal($trailler_weight){
	if($trailler_weight <= 3500){$tax_token_fee = 3500;}
	else if($trailler_weight <= 7500){								
								$xtra_weight = $trailler_weight - 3500;
								$xtra_per_500_kg = $xtra_weight / 500;
								$extra_fee_per_500_kg = $xtra_per_500_kg * 500;
...
function ait_cal($trailler_weight){
	if($trailler_weight <= 1500){$ait_fee = 3000;}
	else if($trailler_weight <= 5000){$ait_fee = 7500;}
	else if($trailler_weight > 5000){$ait_fee = 12500;}
	else{$ait_fee = "";}
...
$reg_non_hire_fee = reg_non_hire_cal($trailler_weight);
...
$tax_token_hire_fee = tax_token_hire_cal($trailler_weight);
...
$ait_fee = ait_cal($trailler_weight);
$ait_total = $ait_fee;
...
<div class="trailler_output_area">
	<div class="container trailler_output">
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<table id="traillerTable" class="table table-condensed">
					<tr class="active">
