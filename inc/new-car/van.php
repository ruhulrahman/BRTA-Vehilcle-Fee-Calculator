...
?>
<div class="van_input_area">
	<div class="container van_input">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="first">Van/Covered Van Fee Calculation</h2>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<form id="van_form" action="<?php htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon">Van/Covered Van Weight</div>
							<input class="form-control" min="1000" max="15000" type="number" name="weight" <?php if($van_weight == ""){}else{ echo "value = ".$van_weight;}?> id="" placeholder="Enter your Van/Covered Van Weight"/>
...
function reg_non_hire_cal($van_weight){
	if($van_weight <= 3500){$reg_fee = 28000;}
	else if($van_weight <= 7500){$reg_fee = 56000;}
	else if($van_weight <= 12500){$reg_fee = 70000;}
	else if($van_weight > 12500){$reg_fee = 77000;}
	else{$reg_fee = "";}
...
function reg_hire_cal($van_weight){
...
function tax_token_hire_cal($van_weight){
	if($van_weight <= 3500){$tax_token_hire_fee = 2000;}
	else if($van_weight <= 7500){
								$xtra_weight = $van_weight - 3500;
	...
	else if($van_weight <= 12500){
								$xtra_weight = $van_weight - 7500;
								$xtra_per_500_kg = $xtra_weight / 500;
...
function tax_token_non_hire_cal($van_weight){
	if($van_weight <= 3500){$tax_token_fee = 3500;}
	else if($van_weight <= 7500){								
								$xtra_weight = $van_weight - 3500;
								$xtra_per_500_kg = $xtra_weight / 500;
								$extra_fee_per_500_kg = $xtra_per_500_kg * 500;
...
$reg_non_hire_fee = reg_non_hire_cal($van_weight);
...
$tax_token_hire_fee = tax_token_hire_cal($van_weight);
...
<div class="van_output_area">
	<div class="container van_output">
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<table id="vanTable" class="table table-condensed">
					<tr class="active">
