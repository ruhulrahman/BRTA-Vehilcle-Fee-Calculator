...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	$tank_lorry_weight = $_POST['weight'];
...
?>
<div class="tank_lorry_input_area">
	<div class="container tank_lorry_input">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="first">Tank Lorry Fee Calculation</h2>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<form id="tank_lorry_form" action="<?php htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
					<div class="form-group">
						<div class="input-group">
							<div class="input-group-addon">Tank Lorry Weight</div>
							<input class="form-control"  min="1000" max="15000" type="number" name="weight" <?php if($tank_lorry_weight == ""){}else{ echo "value = ".$tank_lorry_weight;}?> id="" placeholder="Enter your Tank Lorry Weight"/>
...
function reg_non_hire_cal($tank_lorry_weight){
	if($tank_lorry_weight <= 3500){$reg_fee = 28000;}
	else if($tank_lorry_weight <= 7500){$reg_fee = 56000;}
	else if($tank_lorry_weight <= 12500){$reg_fee = 70000;}
	else if($tank_lorry_weight > 12500){$reg_fee = 77000;}
	else{$reg_fee = "";}
...
function reg_hire_cal($tank_lorry_weight){
	if($tank_lorry_weight <= 3500){$reg_hire_fee = 4600;}
	else if($tank_lorry_weight <= 7500){$reg_hire_fee = 13800;}
	else if($tank_lorry_weight <= 12500){$reg_hire_fee = 17250;}
	else if($tank_lorry_weight > 12500){$reg_hire_fee = 23000;}
	else{$reg_hire_fee = "";}
...
function tax_token_hire_cal($tank_lorry_weight){
	if($tank_lorry_weight <= 3500){$tax_token_hire_fee = 2000;}
	else if($tank_lorry_weight <= 7500){
								$xtra_weight = $tank_lorry_weight - 3500;
	...
	else if($tank_lorry_weight <= 12500){
								$xtra_weight = $tank_lorry_weight - 7500;
								$xtra_per_500_kg = $xtra_weight / 500;
								...
								$xtra_weight = $tank_lorry_weight - 12500;
								$xtra_per_500_kg = $xtra_weight / 500;
								$extra_fee_per_500_kg = $xtra_per_500_kg * 500;
...
function tax_token_non_hire_cal($tank_lorry_weight){
	if($tank_lorry_weight <= 3500){$tax_token_fee = 3500;}
	else if($tank_lorry_weight <= 7500){								
								$xtra_weight = $tank_lorry_weight - 3500;
								$xtra_per_500_kg = $xtra_weight / 500;
								$extra_fee_per_500_kg = $xtra_per_500_kg * 500;
...
function ait_cal($tank_lorry_weight){
	if($tank_lorry_weight <= 1500){$ait_fee = 3000;}
	else if($tank_lorry_weight <= 5000){$ait_fee = 7500;}
	else if($tank_lorry_weight > 5000){$ait_fee = 12500;}
	else{$ait_fee = "";}
...
$reg_non_hire_fee = reg_non_hire_cal($tank_lorry_weight);
...
$tax_token_hire_fee = tax_token_hire_cal($tank_lorry_weight);
...
$ait_fee = ait_cal($tank_lorry_weight);
$ait_total = $ait_fee;
...
<div class="tank_lorry_output_area">
	<div class="container tank_lorry_output">
		<div class="row">
			<div class="col-lg-3"></div>
			<div class="col-lg-6">
				<table id="tank_lorryTable" class="table table-condensed">
					<tr class="active">
